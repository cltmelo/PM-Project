...
def initialize_individual(
    ...
    for activity in activities:
        ...
        if pred_set:
            ...
            binding_sets = _generate_random_binding_sets(pred_set, num_bindings)
    ...
    for activity in activities:
        ...
        if succ_set:
            ...
            num_bindings = random.randint(1, max_bindings_per_activity)
            binding_sets = _generate_random_binding_sets(succ_set, num_bindings)
...
def _extract_direct_follows(
    ...
    Args:
        ...
        activities: List of activity names to consider for follow relationships.
            Relationships are only stored if both activities in the pair
    ...
    for case_id, case_df in grouped:
        # Get activity sequence for this case (all activities, no filtering)
        ...
        for i in range(len(case_activities) - 1):
            ...
            # Only store relationship if both activities are in the target list
            if current_activity in activity_set and next_activity in activity_set:
...
def _generate_random_binding_sets(
    ...
    Note:
        The size of each binding set is randomly selected between 1 and
        len(source_activities), allowing full utilization of available
        source activities without artificial caps.
    ...
    Example:
        >>> sources = {'A', 'B', 'C'}
        >>> bindings = _generate_random_binding_sets(sources, 2)
        >>> len(bindings)
        2
    ...
    for _ in range(num_bindings):
        # Randomly select size for this binding set (at least 1, at most all available)
        binding_size = random.randint(1, len(source_list))
